...
# ─── Helper: reference-based correctness grader (cho Cohen κ) ─────────────────

def grade_answer_vs_reference(question: str, model_answer: str, reference: str) -> int:
    """Chấm 1 answer là ĐÚNG (1) hay SAI (0) dựa trên đáp án chuẩn (reference).

    Khác với pairwise_judge (so 2 answer trên accuracy/completeness/conciseness),
    hàm này chấm tính ĐÚNG/SAI so với ground truth — khớp với cách human gán nhãn
    (human_label = 1 nếu model answer đúng & đầy đủ, 0 nếu sai/thiếu).
    """
    PROMPT = '''Bạn là giám khảo chấm câu trả lời của hệ thống RAG về chính sách nhân sự.
...
Câu trả lời của hệ thống cần chấm:
{answer}

Hãy chấm câu trả lời của hệ thống là ĐÚNG hay SAI so với đáp án chuẩn.
- ĐÚNG (label=1): khớp thông tin cốt lõi với đáp án chuẩn, không mâu thuẫn, không thiếu ý quan trọng làm sai bản chất.
- SAI (label=0): mâu thuẫn với đáp án chuẩn, sai con số/đối tượng, hoặc thiếu ý cốt lõi dẫn đến hiểu sai.

Chỉ trả lời JSON: {{"label": 1 hoặc 0, "reason": "ngắn gọn"}}'''
    try:
        from openai import OpenAI
        client = OpenAI()
        resp = client.chat.completions.create(
            model=JUDGE_MODEL,
            messages=[
                {"role": "system", "content": "Bạn là giám khảo nghiêm khắc. Chỉ trả lời JSON."},
                {"role": "user", "content": PROMPT.format(
                    question=question, reference=reference, answer=model_answer)},
            ],
            response_format={"type": "json_object"},
            temperature=0,
        )
        ...
        print(f"  ⚠️  grade_answer_vs_reference failed: {e}")
        return 0


def save_phase_b_report(judge_results: list[JudgeResult], agreement: list[dict],
                        kappa: float, bias: dict,
    ...
    interp = ("almost perfect" if kappa > 0.8 else "substantial" if kappa > 0.6 else
              "moderate" if kappa > 0.4 else "fair" if kappa > 0.2 else
              "slight" if kappa > 0 else "poor")
    report = {
        "judge_model": JUDGE_MODEL,
        "cohen_kappa": round(kappa, 4),
        "kappa_interpretation": interp,
        "kappa_above_0.6": kappa > 0.6,
...
if __name__ == "__main__":
    # --- Load human labels + ground truth map ---
    with open(HUMAN_LABELS_PATH, encoding="utf-8") as f:
        human_data = json.load(f)
    from config import TEST_SET_PATH
    with open(TEST_SET_PATH, encoding="utf-8") as f:
        gt_map = {item["id"]: item["ground_truth"] for item in json.load(f)}

    human_labels = [item["human_label"] for item in human_data]
    print(f"Human labels loaded: {len(human_labels)} questions")

    # --- Run judge on the same 10 questions to get judge_labels (vs reference) ---
    print("\nRunning reference-based grader + swap-and-average on 10 questions...")
    judge_labels: list[int] = []
    swap_results: list[JudgeResult] = []
    agreement: list[dict] = []
    for item, h in zip(human_data, human_labels):
        qid = item["question_id"]
        question = item["question"]
        model_ans = item["model_answer"]
        reference = gt_map.get(qid, "")

        jl = grade_answer_vs_reference(question, model_ans, reference)
        judge_labels.append(jl)

        # swap-and-average (model_answer vs ground_truth) cho bias analysis
        sr = swap_and_average(question, model_ans, reference)
        swap_results.append(sr)

        agreement.append({
            "question_id": qid, "human_label": h, "judge_label": jl,
            "agree": h == jl,
        })
        print(f"  Q{qid}: human={h} judge={jl} {'✓' if h == jl else '✗'}")

    # --- Cohen's κ ---
    kappa = cohen_kappa(judge_labels, human_labels)
    print(f"\nCohen's κ = {kappa:.3f}  ({'≥0.6 ✓ bonus!' if kappa > 0.6 else '<0.6'})")

    # --- Bias report (from swap results) ---
    bias = bias_report(swap_results)
    print(f"Position bias rate: {bias['position_bias_rate']}  "
          f"Verbosity bias: {bias['verbosity_bias']}")

    save_phase_b_report(swap_results, agreement, kappa, bias)
