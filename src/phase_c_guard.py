...
# PII thật sự cần chặn (loại bỏ nhiễu PERSON/US_* từ model NER tiếng Anh trên text tiếng Việt).
...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    # Chỉ giữ PII thật sự, đủ tin cậy. Model spacy tiếng Anh (en_core_web_lg)
    # hay nhận nhầm chữ tiếng Việt thành PERSON và các recognizer US_* bắn nhiễu
    # với score rất thấp → lọc theo allowlist + ngưỡng score để tránh false positive.
    results = [r for r in raw if r.entity_type in PII_ENTITIES and r.score >= PII_SCORE_THRESHOLD]
    if not results:
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    response = result["content"] if isinstance(result, dict) else str(result)
    # NeMo từ chối bằng cách trả về refuse message được định nghĩa trong rails.co
    refuse_keywords = ["xin lỗi", "không thể", "không được phép", "không nằm trong",
                       "i cannot", "i'm sorry", "i am sorry", "can't help"]
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    response = result["content"] if isinstance(result, dict) else str(result)
    refuse_keywords = ["xin lỗi", "không thể cung cấp", "không thể", "i cannot"]
...
def run_adversarial_suite(adversarial_set: list[dict], rails=None,
    ...
    """
    # Khởi tạo engines một lần để tái sử dụng qua toàn bộ suite (tránh reload chậm)
    if analyzer is None or anonymizer is None:
        analyzer, anonymizer = setup_presidio()
    if rails is None:
        rails = setup_nemo_rails()
...
def measure_p95_latency(test_inputs: list[str], n_runs: int = 20,
    ...
    async def _measure():
        # Lặp lại test_inputs nếu cần để đủ n_runs mẫu
        if not test_inputs:
            return
        samples = [test_inputs[i % len(test_inputs)] for i in range(n_runs)]
        for text in samples:
...
def save_phase_c_report(pii_demo: dict, suite: list[dict], latency: dict,
    ...
    for r in suite:
        c = by_category.setdefault(r["category"], {"passed": 0, "total": 0})
        c["total"] += 1
...
if __name__ == "__main__":
    # Khởi tạo engines MỘT lần, tái sử dụng cho mọi layer (tránh reload chậm)
    print("Initializing Presidio + NeMo Guardrails...")
    analyzer, anonymizer = setup_presidio()
    rails = setup_nemo_rails()
    ...
    suite = run_adversarial_suite(adversarial_set, rails, analyzer, anonymizer)
    passed = sum(1 for r in suite if r["passed"]) if suite else 0
    print(f"Adversarial suite: {passed}/{len(suite)} passed "
          f"({passed/len(suite)*100:.0f}%)" if suite else "no results")
    ...
    save_phase_c_report(pii_demo, suite, latency)
