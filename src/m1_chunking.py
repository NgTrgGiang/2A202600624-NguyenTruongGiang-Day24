...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    # Tách câu: kết thúc bằng . ! ? hoặc ngắt đoạn \n\n
    ...
    for i in range(1, len(sentences)):
        if cosine_sim(embeddings[i - 1], embeddings[i]) < threshold:
            groups.append(" ".join(current))   # đổi chủ đề → chốt chunk
            current = [sentences[i]]
        else:
            current.append(sentences[i])        # cùng chủ đề → gộp tiếp
    groups.append(" ".join(current))

    return [Chunk(text=g, metadata={**metadata, "strategy": "semantic", "chunk_index": i})
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    def _flush(content: str) -> None:
        """Chốt 1 parent rồi cắt nó thành nhiều child (mỗi child ≤ child_size)."""
        content = content.strip()
        if not content:
            return
        pid = f"parent_{len(parents)}"
        parents.append(Chunk(text=content,
                             metadata={**metadata, "chunk_type": "parent", "parent_id": pid}))
        # Cắt parent thành child theo câu, đóng gói tới khi gần đầy child_size
        sentences = re.split(r"(?<=[.!?])\s+", content)
        buf = ""
        for sent in sentences:
            if len(buf) + len(sent) > child_size and buf:
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    # Tách giữ lại header (capturing group) → [text_trước, header, text_sau, header, ...]
