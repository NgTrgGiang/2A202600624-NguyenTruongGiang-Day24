    ...
    try:
        ...
        def _agg(name: str) -> float:
            if name in df.columns:
                m = df[name].mean()           # mean bỏ qua NaN
                return float(m) if m == m else 0.0
            return 0.0

        out = {name: _agg(name) for name in metric_names}
...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    for r in eval_results:
        ...
        worst_metric = min(metrics, key=metrics.get)   # metric thấp nhất = nghi phạm chính
    ...
    analyzed.sort(key=lambda x: x["avg_score"])   # tệ nhất lên đầu
